...
const miNegocio = document.getElementById('negocio'),
    ...
    miTelefono = document.getElementById('telefono'),
...
var miValorNegocio = document.getElementById('negocio').value,
    ...
    miValorTelefono = document.getElementById('telefono').value,
...
miTelefono.addEventListener('focusin', (event) => {
    event.target.style.background = 'paleturquoise';
});
/**Pierde  */
...
miTelefono.addEventListener('focusout', (event) => {
    event.target.style.background = '';
    aplicaEstiloAlInput(event.target);
});
...
comprasRef.onSnapshot((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
        ...
        <td><button class="btn btn-warning" onclick="editar('${doc.id}','${doc.data().negocio}','${doc.data().variedad}','${doc.data().rangoPrecios}','${doc.data().horario}','${doc.data().telefono}','${doc.data().imagen}')">Editar</button></td>
...
// EDITAR documento
function editar(id, negocio, variedad, rangoPrecios, horario, telefono, imagen) {
    ...
    btnGuardarEdicion.onclick = function () {
        ...
        let negocioEditado = document.getElementById('negocio').value,
            ...
            telefonoEditado = document.getElementById('telefono').value,
...
function resetValidaciones() {
    ...
    setInicialFor(miTelefono, 'Telefono no puede quedar vacio');
}
